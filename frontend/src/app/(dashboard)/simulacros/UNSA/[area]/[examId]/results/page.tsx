...
export default function ResultsPage() {
  ...
  // If there's no exam data (direct access), redirect
  useEffect(() => {
    if (!examData) {
      router.push(`/simulacros/UNSA/${area}`);
    }
  }, [examData, router, area]);
  ...
  const porcentaje = Math.round((score.correctas / totalPreguntas) * 100);
  const circumference = 2 * Math.PI * 45;
  const dashOffset = circumference - (porcentaje / 100) * circumference;

  // Group results by materia
  ...
  userAnswers.forEach((answer) => {
    ...
    if (!resultsByMateria[question.materia]) {
      resultsByMateria[question.materia] = { correctas: 0, total: 0 };
    }
    resultsByMateria[question.materia].total++;
    if (answer.respuesta === question.respuestaCorrecta) {
      resultsByMateria[question.materia].correctas++;
    }
  });
  ...
  const handleRetry = () => {
    const data = examData;
    resetExam();
    // Re-navigate to exam to restart
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 py-4">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-gray-400">{examData.meta.name} — {config?.name}</p>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-8 items-center">
          ...
          <div className="relative w-40 h-40 flex-shrink-0">
            ...
            <div className="absolute inset-0 flex items-center justify-center flex-col">
              <span className="font-mono text-3xl font-bold text-white">{score.correctas}</span>
              <span className="font-mono text-sm text-gray-500">/{totalPreguntas}</span>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-4">
            <div>
              ...
              <p className="text-gray-400 text-sm">
                ...
                <span className="font-mono text-white">{formatTime(tiempoUsado)}</span>
              </p>
            </div>
            ...
            <div className="flex gap-4 flex-wrap">
              <span className="font-mono inline-flex items-center gap-2 bg-neutral-900 px-4 py-2 rounded-lg text-sm font-semibold text-gray-300 border border-neutral-border">
                <span className="text-success">✓</span> {score.correctas} Correctas
              </span>
              <span className="font-mono inline-flex items-center gap-2 bg-neutral-900 px-4 py-2 rounded-lg text-sm font-semibold text-gray-300 border border-neutral-border">
                <span className="text-error">×</span> {score.incorrectas} Incorrectas
              </span>
              <span className="font-mono inline-flex items-center gap-2 bg-neutral-900 px-4 py-2 rounded-lg text-sm font-semibold text-gray-300 border border-neutral-border">
                <span className="text-gray-500">—</span> {score.sinResponder} Sin responder
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="bg-neutral-800 rounded-2xl p-6 border border-neutral-border">
        ...
        <div className="space-y-5">
          {Object.entries(resultsByMateria).map(([materia, data]) => {
